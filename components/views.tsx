  ...
  if (!CONTRACT_ADDRESS)
    return (
      <EmptyPage
        ...
        title="CanonMesh is ready for its contract address."
      >
        <p>
          No fallback dataset exists. Configure{" "}
          <code>NEXT_PUBLIC_CANONMESH_CONTRACT</code> after the StudioNet
          deployment.
        </p>
      </EmptyPage>
    );
        ...
        {creating ? (
          ...
          <>
            <header className="manuscript-header">
              <p className="eyebrow">World {world.id} · canonical folio</p>
              <h1>{world.name}</h1>
              <div className="manuscript-meta">
                <span>version {world.version}</span>
                <span>{world.entry_count} canon</span>
            ...
            <div className="action-band">
              <Link
                ...
                href={`/worlds/${world.id}/canon`}
              >
                Open canon ledger
              </Link>
              <Link
                className="secondary-action"
                href={`/worlds/${world.id}/proposals/new`}
              >
                Propose canon
              </Link>
              <Link
                className="secondary-action"
                href={`/worlds/${world.id}/branches`}
              >
                Branch map
              </Link>
              <Link
                className="secondary-action"
                href={`/worlds/${world.id}/timeline`}
              >
                Timeline
              </Link>
            </div>
          ...
          <ReadState result={stats}>
            {(s) => (
              <>
                ...
                <LedgerRule label="Canon" value={s.entry_count} />
...
export function CanonLedger({ worldId }: { worldId: number }) {
  const [world, setWorld] = useState<ReadResult<World>>();
  const [branches, setBranches] = useState<ReadResult<Branch[]>>();
  const [entries, setEntries] = useState<ReadResult<CanonEntry[]>>();
      ...
      <ReadState result={world}>
        {(w) => (
          <header className="page-heading" data-folio="01">
            <BackLink href="/">World desk</BackLink>
            <p className="eyebrow">{w.name} · canonical ledger</p>
            <h1>Canon, without erasure.</h1>
            <p className="deck">
      ...
      <div className="manuscript-grid">
        <section className="manuscript-main">
          <div className="action-band">
            <Link
              className="primary-action"
              href={`/worlds/${worldId}/proposals/new`}
            >
              Propose canon
            </Link>
            <Link
            ...
            {(items) => {
              ...
              return rows.length ? (
                          ...
                          <div className="entity-row">
                            {keys(e.entity_keys_json).map((k) => (
                              <Link
                                ...
                                href={`/worlds/${worldId}/entities/${encodeURIComponent(k)}`}
                              >
                                @{k}
                              </Link>
                            ))}
                ...
                <div className="loading-sheet">
                  No accepted canon matches this view.
                </div>
              );
            }}
          </ReadState>
  ...
  return (
    <div className="page-shell">
      <header className="page-heading" data-folio="02">
        <BackLink href={`/worlds/${worldId}/canon`}>Canon ledger</BackLink>
        <p className="eyebrow">Entity dossier</p>
        <h1>{entityKey}</h1>
        <p className="deck">
          Accepted canon indexed to this normalized entity key.
        </p>
      </header>
      <ReadState result={entries}>
        {(items) =>
          items.length ? (
            ...
            <div className="loading-sheet">
              No accepted canon is indexed to this entity.
            </div>
          )
        }
      </ReadState>
        ...
        <aside className="marginalia">
          ...
          <div className="margin-section">
            ...
            <p className="entry-statement">
              Same-branch RETCON supersedes only same-branch canon. BRANCH_ONLY
...
export function ProposalComposer({ worldId }: { worldId: number }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      router.push(`/proposals/${confirmed.id}`);
    } catch (e) {
      setTx({
        stage: "error",
        message: formatWriteError(e, tx.hash),
      ...
      <div className="manuscript-grid">
        ...
        <aside className="marginalia">
          <div className="margin-section">
            ...
            <p className="entry-statement">
              {mode === "ADD"
                ? "Coexist with active canon."
                : mode === "RETCON"
                  ? "Replace exact active canon in this same branch."
                  : "Diverge from exact inherited ancestor canon in this child branch."}
            </p>
          </div>
        </aside>
      </div>
  ...
  async function review() {
    try {
      ...
      setTx({
        ...
        hash,
        message:
          "Independent validators are resolving the bounded canon decision.",
      });
      await waitForFinalized(client, hash);
      setTx({
        stage: "confirming",
        ...
        {(proposal) => (
          <>
            <header className="page-heading" data-folio="06">
              <BackLink href={`/worlds/${proposal.world_id}/canon`}>
                Canon ledger
              </BackLink>
              <p className="eyebrow">
...
export function DecisionReceipt({ proposalId }: { proposalId: number }) {
  const [p, setP] = useState<ReadResult<Proposal>>();
  const [e, setE] = useState<ReadResult<CanonEntry>>();
  useEffect(() => {
      ...
      <ReadState result={p}>
        {(proposal) => (
          <article className="receipt-sheet">
            ...
            <section className="receipt-section"><h2>Transaction / session proof</h2><p className="field-help">Transaction hashes and finality are shown only when captured in this browser session; the canonical contract receipt above does not invent them.</p></section>
            {CONTRACT_ADDRESS && (
              <a
                className="secondary-action"
...
export function SemanticSearch({ initialWorld }: { initialWorld?: number }) {
  ...
  const [results, setResults] = useState<ReadResult<RelatedCanon[]>>();
  useEffect(() => {
    void listWorlds().then((r) => {
      setWorlds(r);
      if (r.kind === "AVAILABLE" && r.value.length)
        setWorld((x) => x ?? r.value[0].id);
    });
  }, []);
  useEffect(() => {
    if (world)
  ...
  async function run(e: React.FormEvent) {
    e.preventDefault();
    if (world && branch && q.trim())
      setResults(await searchCanon(world, branch, q.trim(), 8));
  }
  return (
    <div className="page-shell">
      ...
      <div className="search-stage">
        <form className="search-controls" onSubmit={run}>
          ...
          <button className="primary-action">Retrieve canon</button>
          <p className="field-help">
            No server index exists; this calls contract VecDB directly.
          </p>
        </form>
        <section className="search-results">
          <ReadState result={results}>
            {(items) =>
              items.map((r) => (
                <article className="search-hit" key={r.entry_id}>
